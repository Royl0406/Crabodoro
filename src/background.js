...
import { MAX_COIN, SESSION_TIME_MINUTES } from "./Common/utilities.js";
...
async function urlChangeHandler(url, tabId) {
  ...
  let { isDistracted: prevIsDistracted } = await chrome.storage.local.get([
    "isDistracted",
  ]);
  ...
  if (prevIsDistracted !== isDistracted) {
    ...
    if (isDistracted === false) {
      ...
      let { totalDistractedTime, distractedStartTime } =
    ...
    //Became distracted
      ...
      let distractedStartTime = Date.now();
...
let startGame = () => {
  ...
  const TOTAL_TIME_MS =
    SESSION_TIME_MINUTES * SECONDS_PER_MINUTE * MILLISECONDS_PER_SECOND;
