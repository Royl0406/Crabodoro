...
function startUpdateLoop() {
  setInterval(() => {
    chrome.storage.local.get(
      ["startTime", "TOTAL_TIME_MS", "remainingTime", "totalDistractedTime"],
      function (result) {
        let remainingTimeMs = calcRemainingTime(
          result.startTime,
        ...
        displayCoinCount(
          calcCoinEarned(
            result.TOTAL_TIME_MS,
            remainingTimeMs,
            result.totalDistractedTime,
          ),
        );
        totCoinsEarned = calcCoinEarned(
          result.TOTAL_TIME_MS,
          remainingTimeMs,
          result.totalDistractedTime,
        );
...
function displayCoinCount(coinCount) {
  const divCoinCount = document.querySelector("#coin-count");
...
function displayRemainingTime(remainingTime) {
  const divRemainingTime = document.querySelector("#remaining-time");
  divRemainingTime.textContent =
    Math.floor(calcMinutes(remainingTime)) +
    " : " +
    Math.round(calcSeconds(remainingTime));
}
